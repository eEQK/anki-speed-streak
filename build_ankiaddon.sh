...
OUTPUT_NAME="${1:-speed_streak_v1_14.ankiaddon}"
...
for item in "$SOURCE_DIR"/* "$SOURCE_DIR"/.[!.]* "$SOURCE_DIR"/..?*; do
  ...
  [ "$name" = ".." ] && continue
  case "$name" in
    .ankiaddon-build|__pycache__|user_files|.DS_Store|.git|.github|.gitignore|CHANGELOG.md|LICENSE|install_to_anki.ps1|build_ankiaddon.ps1|install_to_anki.sh|build_ankiaddon.sh|generate_web_assets.py|*.ankiaddon|*.zip)
      continue
      ;;
  esac
