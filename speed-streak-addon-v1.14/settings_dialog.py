...
class SettingsDialog(QDialog):
    ...
    def _build_toggle_block(self, parent: QWidget, checkbox: QCheckBox, help_text: str) -> QFrame:
        block = QFrame(parent)
    ...
    def _build_display_style_section(self, parent: QWidget) -> QWidget:
        ...
        self.vibration_only_check.toggled.connect(self._on_vibration_only_toggled)
        ...
        layout.addWidget(
            self._build_toggle_block(
                frame,
                self.vibration_only_check,
                "Turns off streak and timer visuals, disables late buzzes, and keeps only haptics.",
            )
        )
        return frame
    ...
    def _build_performance_section(self, parent: QWidget) -> QWidget:
        ...
        self.vibration_only_check_performance = QCheckBox("Vibration Only Mode", frame)
        self.vibration_only_check_performance.toggled.connect(self._on_vibration_only_toggled)
        layout.addWidget(
            self._build_toggle_block(
                frame,
        ...
        layout.addWidget(
            self._build_toggle_block(
                frame,
                self.vibration_only_check_performance,
                "Linked with the Display Style toggle above. This also reduces visual load by disabling streak and timer visuals.",
            )
        )
        return frame
    ...
    def sync_from_state(self) -> None:
        ...
        try:
            ...
            self._set_vibration_only_checks(not bool(state.visuals_enabled))
    ...
    def _set_vibration_only_checks(self, checked: bool) -> None:
        for checkbox in (
            getattr(self, "vibration_only_check", None),
            getattr(self, "vibration_only_check_performance", None),
        ):
            if checkbox is None:
                continue
    ...
    def _on_vibration_only_toggled(self, checked: bool) -> None:
        if self._syncing:
            return
        self._set_vibration_only_checks(bool(checked))
        self.persist_settings()
    ...
    def persist_settings(self) -> None:
        ...
        self.controller.apply_settings_from_dialog(
            ...
            visuals_enabled=not bool(self.vibration_only_check.isChecked() or self.vibration_only_check_performance.isChecked()),
    ...
    def _sync_theme_label(self) -> None:
        ...
        self.color_value.setText(
            f"Display: {display_label}  |  Render: {render_mode_label(getattr(self.controller, 'render_mode', RENDER_MODE_CLASSIC))}\n"
            "Orb colors: "
